...
import type { AuthDevice } from "../types/models";

class AuthDeviceApi {

  getAuthDevice(authDeviceId: string | number): Promise<Response> {
    if (useApi) {
  ...
  updateAuthdevice(authdevice: Partial<AuthDevice>, authdeviceId: string | number): Promise<Response> | undefined {
    if (useApi) {
      return sendApi(`/api/authdevice/${authdeviceId}`,
        {
          method: "PUT",
  ...
  getAvailableEntrances(): Promise<Response> | undefined {
    if (useApi) {
      return sendApi(`/api/availableEntrances`);
    }
  }

  assignEntrance(authDeviceList: AuthDevice[]): Promise<Response> | undefined {
  ...
  removeEntrance(authDeviceList: AuthDevice[]): Promise<Response> | undefined {
    const toUpdate = authDeviceList.map((dev) => ({
  ...
  enableMasterpin(authdeviceId: string | number): Promise<Response> | undefined {
    if (useApi) { return sendApi(`/api/authdevice/masterpinToTrue/${authdeviceId}`, { method: "PUT" }); }
  }

  disableMasterpin(authdeviceId: string | number): Promise<Response> | undefined {
    if (useApi) { return sendApi(`/api/authdevice/masterpinToFalse/${authdeviceId}`, { method: "PUT" }); }
  }

  deleteAuthdevice(authdeviceId: string | number): Promise<Response> | undefined {
    if (useApi) { return sendApi(`/api/authdevice/delete/${authdeviceId}`, { method: "DELETE" }); }
  }

  resetAuthDevice(authdeviceId: string | number): Promise<Response> | undefined {
    if (useApi) { return sendApi(`/api/authdevice/reset/${authdeviceId}`, { method: "DELETE" }); }
  }

  getAllAuthMethods(): Promise<Response> | undefined {
    if (useApi) { return sendApi(`/api/allAuthMethods`, { method: "GET" }); }
  }

  getAuthenticationSchedules(authDeviceId: string | number): Promise<Response> {
