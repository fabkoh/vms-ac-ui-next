import { useApi, fakeAccessGroups, fakePersons } from "./api-config";
...
class AccessGroupApi {
  ...
  getAccessGroups(): Promise<Response> {
    ...
    accessGroups.forEach((group: any) => {
      const accessGroupId = group.accessGroupId;
      group.persons = fakePersons.filter((p: any) => p.accessGroup == accessGroupId);
    });

    return Promise.resolve(new Response(JSON.stringify(accessGroups), { status: 200 }));
  }

  getAccessGroup(id: number | string): Promise<Response> {
    if (useApi) { return sendApi(`/api/accessgroup/${id}`); }

    const accessGroup: any = { ...fakeAccessGroups.find((group) => group.accessGroupId == id) };

    if (accessGroup) {
      const accessGroupId = accessGroup.accessGroupId;
      accessGroup.persons = fakePersons.filter((p: any) => p.accessGroup == accessGroupId);
  ...
  updateAccessGroup({
    ...
    fakeAccessGroups[index] = updatedAccessGroup as any;
  ...
  deleteAccessGroup(id: number | string): Promise<Response> {
    ...
    fakePersons.forEach((person: any) => {
  ...
  updateAccessGroupStatus(accessGroupId: number | string, isActive: boolean): Promise<Response> {
    if (useApi) {
      if (isActive) {
    ...
    (fakeAccessGroups.find((accessGroup) => accessGroup.accessGroupId == accessGroupId) as any).isActive = isActive;
