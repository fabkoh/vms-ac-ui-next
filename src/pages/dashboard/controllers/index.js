...
import {
  filterControllerByString,
  filterControllerByStringPlaceholder,
...
const ControllerList = () => {
  ...
  const selectedSomeControllers =
    selectedControllers.length > 0 && !selectedAllControllers;
  const handleSelectAllControllers = (e) =>
    setSelectedControllers(
      e.target.checked ? controllers.map((c) => c.controllerId) : []
    );
  const handleSelectFactory = (controllerId) => () => {
    if (selectedControllers.includes(controllerId)) {
      setSelectedControllers(
        selectedControllers.filter((id) => id !== controllerId)
      );
    } else {
  ...
  const paginatedControllers = applyPagination(
    filteredControllers,
    page,
    rowsPerPage
  );
