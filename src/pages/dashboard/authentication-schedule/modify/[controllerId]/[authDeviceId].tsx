...
const ModifyauthMethodSchedule = () => {
  ...
  const getControllerAuthDevices = async () => {
    ...
    data.forEach((c) => {
      ...
      authDeviceArr.sort((a, b) =>
        a.authDeviceDirection.localeCompare(b.authDeviceDirection)
      );
  ...
  const getNewId = () =>
    authMethodScheduleInfoArr
      .map((info) => info.authMethodScheduleId)
      .reduce((a, b) => Math.max(a, b), -1) + 1;

  const addCard = () => {
    const newId = getNewId();
    setauthMethodScheduleInfoArr([
      ...authMethodScheduleInfoArr,
      getEmptyauthMethodScheduleInfo(newId),
    ]);
    setauthMethodScheduleValidationsArr([
      ...authMethodScheduleValidationsArr,
      getEmptyauthMethodScheduleValidations(newId),
    ]);
  };

  // remove card logic
  const removeCard = (id) => {
    const newAuthMethodScheduleInfoArr = authMethodScheduleInfoArr.filter(
      (info) => info.authMethodScheduleId != id
    );
    const newValidations = authMethodScheduleValidationsArr.filter(
      (validation) => validation.authMethodScheduleId != id
    );
  ...
  const changeTextField = (e, id) => {
    ...
    updatedInfo.find((info) => info.authMethodScheduleId == id)[e.target.name] =
      e.target.value;
  ...
  const changeAuthMethod = (e, id) => {
    const updatedInfo = [...authMethodScheduleInfoArr];
    updatedInfo.find((info) => info.authMethodScheduleId == id)["authMethod"] =
      e.target.value;
    setauthMethodScheduleInfoArr(updatedInfo);
  };
  ...
  const changeRrule = (string, id) => {
    const updatedInfo = [...authMethodScheduleInfoArr];
    updatedInfo.find((info) => info.authMethodScheduleId == id)["rrule"] =
      string;
    setauthMethodScheduleInfoArr(updatedInfo);
  };
  //set timestartend
  const changeTimeStart = (start, id) => {
    const updatedInfo = [...authMethodScheduleInfoArr];
    updatedInfo.find((info) => info.authMethodScheduleId == id)["timeStart"] =
      start;
    setauthMethodScheduleInfoArr(updatedInfo);
  ...
  const changeTimeEnd = (end, id) => {
    const updatedInfo = [...authMethodScheduleInfoArr];
    updatedInfo.find((info) => info.authMethodScheduleId == id)["timeEnd"] =
      end;
    setauthMethodScheduleInfoArr(updatedInfo);
    checkTimeEnd(end, id);
  ...
  const checkRruleAndTimeClahses = (
    rrule1,
    timeStart1,
    timeEnd1,
    rrule2,
    ...
    if (rrule1 != undefined && rrule2 != undefined) {
      ...
      const Weekday1 = rrule1
        .slice(rrule1.lastIndexOf("BYDAY=") + 6)
        .split(",");
      const Weekday2 = rrule2
        .slice(rrule2.lastIndexOf("BYDAY=") + 6)
        .split(",");
      ...
      Weekday2.forEach((day2) => {
        ...
        if (Weekday1.some(dayExist)) {
          ...
          if (
            !(
              (timeStart1 > timeEnd2 && timeEnd1 > timeEnd2) ||
              (timeStart1 < timeStart2 && timeEnd1 < timeStart2)
            )
          ) {
  ...
  const checkClashingWeekdays = () => {
    authMethodScheduleInfoArr.map((sch) => {
      ...
      const newRrule = newAuthMethodScheduleInfoArr.find(
        (v) => v.authMethodScheduleId == id
      )["rrule"];
      const NewTimeStart = newAuthMethodScheduleInfoArr.find(
        (v) => v.authMethodScheduleId == id
      )["timeStart"];
      const NewTimeEnd = newAuthMethodScheduleInfoArr.find(
        (v) => v.authMethodScheduleId == id
      )["timeEnd"];

      console.log(NewTimeStart, NewTimeEnd);

      const listOfClashes = newAuthMethodScheduleInfoArr
        .filter(
          (singleAuthMethodSchedule) =>
            singleAuthMethodSchedule.authMethodScheduleId != id
        )
        // return id of clahses
        // check for rrule fisrt, of true, check for time
        .filter((singleAuthMethodSchedule) =>
          checkRruleAndTimeClahses(
            newRrule,
            NewTimeStart,
            NewTimeEnd,
  ...
  const checkTimeEnd = (end, id) => {
    ...
    const tempStartTime = newAccessGroupScheduleInfoArr.find(
      (group) => group.authMethodScheduleId == id
    )["timeStart"];
    ...
    validation.timeEndInvalid =
      formUtils.checkBlank(endTime) || endTime <= tempStartTime;
    // validation.timeEndInvalid = formUtils.checkBlank(endTime);
    // console.log(validation)
    setauthMethodScheduleValidationsArr(newValidations);
  };
  ...
  const changeNameCheck = async (e, id) => {
    ...
    tempAuthMethodScheduleInfoArr.find(
      (group) => group.authMethodScheduleId == id
    ).authMethodScheduleName = authMethodScheduleName;

    // remove submit failed
    // validation.submitFailed = false;

    // check name is blank?
    validation.authMethodScheduleNameBlank = formUtils.checkBlank(
      authMethodScheduleName
    );
    ...
    authMethodScheduleApi
      ...
      .then((res) => {
        if (res.status != 200) {
          ...
          router.replace(
            `/dashboard/controllers/auth-device/details/${controllerId}/${authDeviceId}`
          );
        }
      });
  };
  ...
  const authdeviceEqual = (option, value) =>
    option.authDeviceId == value.authDeviceId;
  ...
  return (
    <>
      <Head>
        <title>Etlas: Modify Authentication Device Schedules</title>
      </Head>
      <Box
        component="main"
        sx={{
      ...
      >
        ...
        <Container maxWidth="xl">
          <Box sx={{ mb: 4 }}>
              ...
              <Link
                color="textPrimary"
                component={NextLink} href={`/dashboard/controllers/auth-device/details/${controllerId}/${authDeviceId}`}
                sx={{
                  alignItems: "center",
                  display: "flex",
                }}
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Modify Authentication Schedule</Typography>
            {/* <Grid container>
                            <Grid item mr={1}>
            ...
            <Alert severity="info" variant="outlined">
              Quick tip : You may apply these schedules to multiple
              authentication devices by selecting more than one authentication
              device{" "}
            </Alert>
          </Box>
          <Grid container alignItems="center" mb={3}>
            ...
            <Grid item xs={11} md={7}>
              <MultipleSelectInput
                ...
                error={Boolean(authDevices.length == 0)}
                helperText={
                  Boolean(authDevices.length == 0) &&
                  "Error : no authentication device selected"
                }
              />
            </Grid>
            </Grid>
            ...
            <Stack spacing={3}>
              {authMethodScheduleInfoArr.map((authMethodScheduleInfo, i) => (
                <EditAuthSchedForm
                  ...
                  authMethodScheduleValidations={
                    authMethodScheduleValidationsArr[i]
                  }
              ...
              <Grid container>
                <Grid item marginRight={3} mb={2}>
                  <Button
                    ...
                    disabled={
                      ...
                      authMethodScheduleValidationsArr.some(
                        // check if validations fail
                        (validation) =>
                          validation.authMethodScheduleNameBlank ||
                          validation.timeEndInvalid ||
                ...
                <Grid item>
                    ...
                    <Button component={NextLink} href={`/dashboard/controllers/auth-device/details/${controllerId}/${authDeviceId}`} size="large" variant="outlined" color="error">
                      Cancel
                    </Button>
                </Grid>
              </Grid>
...
ModifyauthMethodSchedule.getLayout = (page) => (
  <AuthGuard>
