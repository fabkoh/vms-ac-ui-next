...
const EventStringFilterHelper = (event, query) =>
  ...
  (event.eventActionType &&
    stringIn(query, event.eventActionType.eventActionTypeName)) ||
  (event.person &&
    stringIn(
      query,
      event.person.personFirstName + " " + event.person.personLastName
    )) ||
  (event.accessGroup && stringIn(query, event.accessGroup.accessGroupName));

const filterEventsbyString = (event, queryString) =>
  EventStringFilterHelper(event, queryString.toLowerCase());
...
const Logs = () => {
  ...
  const handlePageChange = async (e, newPage) => {
    setPage(newPage);
    if (
      (newPage + 1) * rowsPerPage >= filteredEvents.length &&
      ...
      if (res.status == 200) {
        ...
        if (isMounted()) setEvents(Events.concat(data));
  ...
  const getInfo = useCallback(async () => {
    ...
    // console.log("event count is " + eventsCountJson);
    // console.log("event length is " + Events.length);
    if (Events.length < eventsCountJson) {
      console.log("getEvents activated");
