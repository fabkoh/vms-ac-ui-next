...
// import NEXT_PUBLIC_URI from "../../../../../.env.production.local";
...
const EditVideoRecorder = () => {
  ...
  const [recorderValidationsArr, setRecorderValidationsArr] = useState(
    ids.map((i) => getEmptyRecorderValidations(i))
  );
  ...
  const getVideoRecorder = async (ids) => {
    ...
    const resArr = await Promise.all(
      ids.map((id) => videoRecorderApi.getRecorder(id))
    );
  ...
  const submitForm = async (e) => {
    ...
    try {
      const boolArr = await Promise.all(
        recorderInfoArr.map((recorder) => updateRecorderHelper(recorder))
      );
  ...
  return (
    <>
      <Head>
        <title>Etlas: Edit Video Recorder</title>
      </Head>
      <Box
        component="main"
        sx={{
        ...
        <Container maxWidth="xl">
          <Box sx={{ mb: 4 }}>
            ...
            />
              ...
              <Link
                color="textPrimary"
                component={NextLink} href={videoRecorderListLink}
                sx={{
                  alignItems: "center",
                  display: "flex",
                }}
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Edit Video Recorders</Typography>
          </Box>
            ...
            <Stack spacing={3}>
              ...
              <div>
                ...
                >
                  ...
                  <Button component={NextLink} href={videoRecorderListLink}
                    size="large"
                    sx={{ mr: 3 }}
                    variant="outlined"
                    color="error"
