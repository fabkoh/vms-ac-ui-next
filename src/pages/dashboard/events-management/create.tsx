...
const ModifyEventManagement = () => {
  ...
  const [notificationDisabledOpen, setNotificationDisabledOpen] =
    useState(false);
  ...
  const selectedAllEventsManagement =
    selectedEventsManagement.length ===
    [...new Set(singleErrorMessage.map((e) => e.eventsManagementId))].length;
  ...
  const handleSelectAllEventsManagement = (e) =>
    setSelectedEventsManagement(
      e.target.checked
        ? [...new Set(singleErrorMessage.map((e) => e.eventsManagementId))]
        : []
    );
  ...
  const [customInputEventsSelected, setCustomInputEventsSelected] = useState(
    {}
  ); // List of custom input events (GEN_IN_1, etc) that is selected for validation
  const [customOutputEventsSelected, setCustomOutputEventsSelected] = useState(
    {}
  ); // List of custom output events (GEN_OUT_1, etc) that is selected for validation
  ...
  const getAllInputOutputEvents = useCallback(
    async (forController) => {
  ...
  const getSMSEmailConfig = useCallback(async () => {
    const smsNotificationConfig =
      await notificationConfigApi.getNotificationSMSConfig();
    const emailNotificationConfig =
      await notificationConfigApi.getNotificationEmailConfig();
  ...
  const [eventsManagementInfoArr, setEventsManagementInfoArr] = useState([
    getEmptyEventsManagementInfo(0),
  ]);
  const [eventsManagementValidationsArr, setEventsManagementValidationsArr] =
    useState([getEmptyEventsManagementValidations(0)]);
  ...
  // add card logic
  const getNewId = () =>
    eventsManagementInfoArr
      .map((info) => info.eventsManagementId)
      .reduce((a, b) => Math.max(a, b), -1) + 1;

  const addCard = () => {
    const newId = getNewId();
    setEventsManagementInfoArr([
      ...eventsManagementInfoArr,
      getEmptyEventsManagementInfo(newId),
    ]);
    setEventsManagementValidationsArr([
      ...eventsManagementValidationsArr,
      getEmptyEventsManagementValidations(newId),
    ]);
  };

  // remove card logic
  const removeCard = (id) => {
    const newEventsManagementInfoArr = eventsManagementInfoArr.filter(
      (info) => info.eventsManagementId != id
    );
    const newValidations = eventsManagementValidationsArr.filter(
      (validation) => validation.eventsManagementId != id
    );
  ...
  const changeTextField = (e, id) => {
    ...
    updatedInfo.find((info) => info.eventsManagementId == id)[e.target.name] =
      e.target.value;
  ...
  const changeNameCheck = async (e, id) => {
    ...
    newEventsManagementInfoArr.find(
      (group) => group.eventsManagementId == id
    ).eventsManagementName = eventsManagementName;

    // remove submit failed
    validation.submitFailed = false;

    // check name is blank?
    validation.eventsManagementNameBlank =
      formUtils.checkBlank(eventsManagementName);
  ...
  const submitReplaceAll = () => {
    Promise.resolve(
      eventsManagementApi.replaceEventsManagement(
        eventsManagementInfoArr,
        entrances,
        controllers
      )
  ...
  const submitAddAll = () => {
    Promise.resolve(
      eventsManagementApi.addEventsManagement(
        eventsManagementInfoArr,
        entrances,
        controllers
      )
  ...
  const entranceControllerFilter = (entranceController, state) => {
    ...
    return entranceController.filter(
      (e) =>
  ...
  const eventActionOutputEqual = (option, value) =>
    option.eventActionOutputId == value.eventActionOutputId;
  ...
  const updateEventManagementDefaultTitle = () => {
    ...
    for (let i = 0; i < updatedInfo.length; i++) {
      updatedInfo[i].eventsManagementDefaultTitle =
        "Event Management " +
        updatedInfo[i].eventsManagementName +
        " Triggered";
    }
    setEventsManagementInfoArr(updatedInfo);
  };
  ...
  const updateEventManagementDefaultContent = () => {
    ...
    for (let i = 0; i < updatedInfo.length; i++) {
      ...
      for (let j = 0; j < currInputEvents.length; j++) {
        const inputEventId =
          currInputEvents[j].eventActionInputType.eventActionInputId;
        const inputEventEntity = inputEvents.find(
          (e) => e.eventActionInputId == inputEventId
        );
        if (currInputEvents[j].timerDuration) {
          inputStr +=
            inputEventEntity.eventActionInputName +
            " for " +
            currInputEvents[j].timerDuration +
            " second(s), ";
        } else {
      ...
      updatedInfo[i].eventsManagementDefaultContent =
        "with the following triggers: " + toBeAdded + " has been triggered";
    }
    setEventsManagementInfoArr(updatedInfo);
  };
  ...
  const changeEntranceController = (newValue) => {
    ...
    // const updatedInfo = [...eventsManagementInfoArr];
    const entrancesOnly = newValue.filter(
      (e) => e.entranceId != undefined || e.entranceId != null
    );
    const controllersOnly = newValue.filter(
      (c) => c.controllerId != undefined || c.controllerId != null
    );
  ...
  const changeInputEventsWithoutTimer = (newValue, id) => {
    ...
    const inputEventToBeAdded = inputEvents.find(
      (e) => e.eventActionInputId == newValue
    );
    const eventManagementToBeUpdatedInputEvents =
      eventManagementToBeUpdated["inputEvents"];
    let newInputEvents = [];
    ...
    for (let i = 0; i < eventsManagementInfoArr.length; i++) {
      ...
      for (let j = 0; j < currOutputActions.length; j++) {
        if (currOutputActions[j].timerDuration) {
          const outputEventId =
            currOutputActions[j].eventActionOutputType.eventActionOutputId;
          const outputEventEntity = outputEvents.find(
            (e) => e.eventActionOutputId == outputEventId
          );
          if (outputEventEntity?.eventActionOutputName == "GEN_OUT_1") {
            if (selectedOutputEvents["GEN_OUT_1"] == undefined) {
              selectedOutputEvents["GEN_OUT_1"] = [
                currEventManagement.eventsManagementId,
              ];
            } else {
              selectedOutputEvents["GEN_OUT_1"].push(
                currEventManagement.eventsManagementId
              );
            }
          } else if (outputEventEntity?.eventActionOutputName == "GEN_OUT_2") {
            if (selectedOutputEvents["GEN_OUT_2"] == undefined) {
              selectedOutputEvents["GEN_OUT_2"] = [
                currEventManagement.eventsManagementId,
              ];
            } else {
              selectedOutputEvents["GEN_OUT_2"].push(
                currEventManagement.eventsManagementId
              );
            }
          } else if (outputEventEntity?.eventActionOutputName == "GEN_OUT_3") {
            if (selectedOutputEvents["GEN_OUT_3"] == undefined) {
              selectedOutputEvents["GEN_OUT_3"] = [
                currEventManagement.eventsManagementId,
              ];
            } else {
              selectedOutputEvents["GEN_OUT_3"].push(
                currEventManagement.eventsManagementId
              );
            }
          }
        ...
        if (genOut2s != undefined) {
          for (let i = 0; i < value.length; i++) {
            ...
            for (let i = 0; i < genOut2s.length; i++) {
              const validation = newValidations.find(
                (v) => v.eventsManagementId == genOut2s[i]
              );
              validation.eventsManagementOutputActionsConflict = true;
            }
          }
        ...
        if (genOut3s != undefined) {
          for (let i = 0; i < value.length; i++) {
            ...
            for (let i = 0; i < genOut3s.length; i++) {
              const validation = newValidations.find(
                (v) => v.eventsManagementId == genOut3s[i]
              );
              validation.eventsManagementOutputActionsConflict = true;
            }
          }
  ...
  const changeOutputActionsWithoutTimer = (newValue, id) => {
    ...
    let eventManagementToBeUpdated = updatedInfo.find(
      (info) => info.eventsManagementId == id
    );
    const eventManagementToBeUpdatedOutputActions =
      eventManagementToBeUpdated["outputActions"];
      ...
      if (eventManagementToBeUpdated["eventsManagementEmail"] === null) {
        ...
        eventManagementToBeUpdated["eventsManagementEmail"] = {
          ...
          eventsManagementEmailContent:
            eventManagementToBeUpdated.eventsManagementDefaultContent,
          eventsManagementEmailTitle:
            eventManagementToBeUpdated.eventsManagementDefaultTitle,
        };
        let newNotificationEmails = { ...notificationEmails };
    ...
    setEventsManagementInfoArr(updatedInfo);
    setOutputActionsWithoutTimer({
      ...outputActionsWithoutTimer,
      [id]: newValue,
    });
    ...
    validation.eventsManagementOutputActionsEmpty =
      newOutputActions.length === 0;
    if (
      newValueMapped.filter(
        (i) =>
          i.eventActionOutputType.eventActionOutputId == null ||
          i.eventActionOutputType.eventActionOutputId == undefined
      ).length > 0
    ) {
    ...
    if (!hasNotificationEmailsOutput) {
      ...
      validation.eventsManagementEmailRecipientsEmpty =
        notificationEmails[id]?.eventsManagementEmailRecipients.length === 0 ||
        notificationEmails[id] === undefined;
    }
    if (!hasNotificationSMSsOutput) {
      ...
      validation.eventsManagementSMSRecipientsEmpty =
        notificationSMSs[id]?.eventsManagementSMSRecipients.length === 0 ||
        notificationSMSs[id] === undefined;
    }
    setEventsManagementValidationsArr(newValidations);
  };
  ...
  const changeInputEventsWithTimer = (newValue, id) => {
    ...
    for (let j = 0; j < eventManagementToBeUpdatedInputEvents.length; j++) {
      if (
        eventManagementToBeUpdatedInputEvents[j].timerDuration == null ||
        eventManagementToBeUpdatedInputEvents[j].timerDuration == undefined
      ) {
        newInputEvents.push(eventManagementToBeUpdatedInputEvents[j]);
      }
    }
    ...
    if (
      newValue.filter(
        (i) =>
          i.eventActionInputType.eventActionInputId == null ||
          i.eventActionInputType.eventActionInputId == undefined
      ).length > 0
    ) {
      validation.eventsManagementInputEventsInvalidId = true;
    } else {
  ...
  const changeOutputActionsWithTimer = (newValue, id) => {
    ...
    const eventManagementToBeUpdatedOutputEvents =
      eventManagementToBeUpdated["outputActions"];
    let newOutputActions = [];
    for (let j = 0; j < eventManagementToBeUpdatedOutputEvents.length; j++) {
      if (
        eventManagementToBeUpdatedOutputEvents[j].timerDuration == null ||
        eventManagementToBeUpdatedOutputEvents[j].timerDuration == undefined
      ) {
  ...
  const changeNotificationEmails = (newValue, id) => {
    ...
    if (newValue.useDefaultEmails) {
      newEmailValue = {
        eventsManagementEmailRecipients: newRecipientsCSV,
        eventsManagementEmailContent:
          updatedInfo[0].eventsManagementDefaultContent,
        eventsManagementEmailTitle: newValue.eventsManagementEmailTitle,
        useDefaultEmails: newValue.useDefaultEmails,
      };
    } else {
      newEmailValue = {
        eventsManagementEmailRecipients: newRecipientsCSV,
        eventsManagementEmailContent: newValue.eventsManagementEmailContent,
        eventsManagementEmailTitle: newValue.eventsManagementEmailTitle,
        useDefaultEmails: newValue.useDefaultEmails,
      };
    }
  ...
  const changeNotificationSMSs = (newValue, id) => {
    ...
    if (newValue.useDefaultSMS) {
      newSMSValue = {
        eventsManagementSMSRecipients: newRecipientsCSV,
        eventsManagementSMSContent:
          updatedInfo[0].eventsManagementDefaultContent,
        useDefaultSMS: newValue.useDefaultSMS,
      };
    } else {
      newSMSValue = {
        eventsManagementSMSRecipients: newRecipientsCSV,
        eventsManagementSMSContent: newValue.eventsManagementSMSContent,
        useDefaultSMS: newValue.useDefaultSMS,
      };
    }
  ...
  return (
    <>
      <Head>
        <title>Etlas: Create Events Management</title>
      </Head>
      <Box
        component="main"
        sx={{
        ...
        <Container maxWidth="xl">
          <Box sx={{ mb: 4 }}>
            ...
            <ConfirmNotificationDisabledSubmit
              ...
              submitEventsManagement={
                action === "addon" ? () => addOn(true) : () => replaceAll(true)
              }
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Create Events Management</Typography>
            {/* <Grid container
                            marginLeft={0.5}
                            marginBottom={1}>
                            <Grid item
            ...
            <Box marginTop={1}>
              <Alert severity="info" variant="outlined">
                Quick tip : You may select more than one entrance/controller to
                apply these actions to multiple entrances/controllers
              </Alert>
              </Box>
            </Box>
          <Grid container alignItems="center" mb={3}>
            ...
            <Grid item xs={11} md={7}>
              <MultipleSelectInput
                ...
                error={Boolean(entrancesControllers.length == 0)}
                helperText={
                  (Boolean(entrancesControllers.length == 0) &&
                    "Error : no entrance/ controller selected") ||
                  (Boolean(controllers.length != 0) &&
                    "Note: When a controller is selected, some trigger/action(s) may be unavailable. You may only create events management for entrances with those input/output types")
                }
              />
            </Grid>
          </Grid>
          <Stack spacing={3}>
              ...
              return (
                <EditEventManagementForm
                  ...
                  eventsManagementValidations={
                    eventsManagementValidationsArr[i]
                  }
                  changeTextField={onNameChangeFactory(
                    eventsManagementInfo.eventsManagementId
                  )}
                  changeTriggerSchedules={changeTriggerSchedules}
                  checkAnyBeginForEventManagement={checkAnyBeginForEventManagement(
                    eventsManagementInfo.eventsManagementId
                  )}
                  checkAnyUntilForEventManagement={checkAnyUntilForEventManagement(
                    eventsManagementInfo.eventsManagementId
                  )}
                  checkAnyTimeStartForEventManagement={checkAnyTimeStartForEventManagement(
                    eventsManagementInfo.eventsManagementId
                  )}
                  checkAnyTimeEndForEventManagement={checkAnyTimeEndForEventManagement(
                    eventsManagementInfo.eventsManagementId
                  )}
            ...
            <Grid container>
              <Grid item marginRight={3} mb={2}>
                <Button
                  ...
                  disabled={
                    ...
                    eventsManagementValidationsArr.some(
              ...
              <Grid item>
                  ...
                  <Button component={NextLink} href="/dashboard/events-management/"
                    size="large"
                    variant="outlined"
                    color="error"
                    onClick={() => router.back()}
