...
const EditEntrances = () => {
  ...
  const getEntrances = async (ids) => {
    ...
    const resArr = await Promise.all(
      ids.map((id) => entranceApi.getEntrance(id))
    );
  ...
  const getAccessGroupsEntrances = async (accGrpEntrances) => {
    ...
    const resArr = await Promise.all(
      entrances.map((e) =>
        accessGroupEntranceApi.getAccessGroupWhereEntranceId(e.entranceId)
      )
    );
    ...
    resArr.forEach((res, i) => {
      if (res.status == 200) {
        ...
        toast.error(
          `Access Groups for ${entrances[i].entranceName} not loaded. Please clear to prevent changes`
        );
      }
    });
    ...
    bodyArr.forEach((body, i) => {
      entrances[successfulResIndex[i]].accessGroups = body.map(
        (group) => group.accessGroup
      );
    });
  ...
  const submitForm = async (e) => {
    ...
    const resArr = await Promise.all(
      entranceInfoArr.map((e) => entranceApi.updateEntrance(e))
    );
    ...
    setEntranceValidationsArr(
      entranceValidationsArr.filter((e, i) => !successStatus[i])
    );
    setSubmitted(false);
  };
  ...
  return (
    <>
      <Head>
        <title>Etlas: Edit Entrances</title>
      </Head>
      <Box
        component="main"
        sx={{
      ...
      >
        <Container maxWidth="xl">
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Edit Entrances</Typography>
          </Box>
            ...
            <Stack spacing={3}>
              ...
              <Grid container>
                ...
                <Grid item>
                    ...
                    <Button component={NextLink} href="/dashboard/entrances/"
                      size="large"
                      variant="outlined"
                      color="error"
                      onClick={() => router.back()}
