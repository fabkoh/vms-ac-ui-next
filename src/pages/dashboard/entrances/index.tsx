...
import {
  filterEntranceByStringPlaceholder,
  filterEntranceByStatus,
  filterEntranceByCurrStatus,
  filterEntranceByString,
  entranceCreateLink,
...
const EntranceList = () => {
  ...
  const getAccessGroupsLocal = useCallback(
    async (entrances) => {
      const newEntrances = [...entrances];
      const resArr = await Promise.all(
        newEntrances.map((entrance) =>
          accessGroupEntrance.getAccessGroupWhereEntranceId(entrance.entranceId)
        )
      );
      const successArr = resArr.map((res) => res.status == 200);
      const serverDownFailArr = resArr.filter(
        (res) => res.status == serverDownCode
      );
      ...
      if (successArr.some((success) => !success)) {
        // some res fail
        toast.error("Error loading access groups info");
      }
      const jsonArr = await Promise.all(resArr.map((res) => res.json()));
      newEntrances.forEach(
        (entrance, i) =>
          (entrance.accessGroups = successArr[i] ? jsonArr[i] : [])
      );
  ...
  const handleSelectAllEntrances = (e) =>
    setSelectedEntrances(
      e.target.checked ? entrances.map((e) => e.entranceId) : []
    );
  ...
  const paginatedEntrances = applyPagination(
    filteredEntrances,
    page,
    rowsPerPage
  );
  ...
  const handleMultipleActivationUpdate = (updateStatus) =>
    openStatusUpdateDialog([...selectedEntrances], updateStatus);
  const handleMultipleUnlock = () =>
    openUnlockDialogFunc([...selectedEntrances]);
  ...
  const handleUnlockApiCall = async (entranceIds) => {
    ...
    const resArr = await Promise.all(
      entranceIds.map((entranceId) =>
        entranceApi.manuallyUnlockEntrance(entranceId)
      )
    );
    ...
    if (someFailed) {
      toast.error("Failed to unlock some entrances");
    }
    if (successCount) {
      toast.success(
        "Successfully unlock " +
          (successCount > 1 ? successCount + " entrances" : "1 entrance")
      );
    }
  };
  ...
  const handleStatusUpdate = async (entranceIds, updatedStatus) => {
    ...
    if (successCount) {
      toast.success(
        "Successfully " +
          (updatedStatus ? "activated" : "deactivate") +
          " " +
          (successCount > 1 ? successCount + " entrances" : "1 entrance")
      );
    }
            ...
            <Grid container justifyContent="space-between" spacing={3}>
              ...
              <Grid item>
                ...
                >
                    ...
                    <MenuItem component={NextLink} href={entranceCreateLink} disableRipple>
                      <Add />
                      &#8288;Create
                    </MenuItem>
                  
                    <MenuItem component={NextLink} href={getEntranceIdsEditLink(selectedEntrances)} disableRipple disabled={actionDisabled}>
                      <Edit />
                      &#8288;Edit
                    </MenuItem>
                  <MenuItem
