...
import {
  filterAccessGroupByStringPlaceholder,
  filterAccessGroupByString,
...
const AccessGroupList = () => {
  ...
  const getAccessGroupLocal = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      const entrancesRes = await Promise.all(
        data.map((group) =>
      ...
      const serverDownRes = entrancesRes.filter(
      ...
      const accessGroupWithScheduleStatus =
        await getAccessGroupInScheduleStatus();
      const accGroupWithSchedStatus = data.map((accessGroup) => {
        return {
          ...accessGroup,
          isInSchedule:
            accessGroupWithScheduleStatus[accessGroup.accessGroupId],
        };
      });
      const entrancesData = await Promise.all(
        entrancesRes.map((res) => res.json())
      );
      if (isMounted()) {
        accGroupWithSchedStatus.forEach(
          (group, i) => (group.entrances = entrancesData[i])
        );
  ...
  const handleSelectAllAccessGroup = (event) => {
    setSelectedAccessGroup(
      event.target.checked
        ? accessGroup.map((accGroup) => accGroup.accessGroupId)
        : []
    );
  };
  ...
  const selectedSomeAccessGroup =
    selectedAccessGroup.length > 0 &&
    selectedAccessGroup.length < accessGroup.length;
  const selectedAllAccessGroup =
    selectedAccessGroup.length === accessGroup.length;
  ...
  const openStatusUpdateDialog = (accessGroupIds, updatedStatus) => {
    console.log("openStatusUpdateDialog");
  ...
  const handleMultipleUpdate = (updateStatus) =>
    openStatusUpdateDialog([...selectedAccessGroup], updateStatus);
            ...
            <Grid container justifyContent="space-between" spacing={3}>
              ...
              <Grid item>
                ...
                >
                    ...
                    <MenuItem component={NextLink} href={"/dashboard/access-groups/create"} disableRipple>
                      <AddIcon />
                      &#8288;Create
                    </MenuItem>
                    ...
                    <MenuItem component={NextLink} href={{
                      ...
                      query: {
                        ids: encodeURIComponent(
                          JSON.stringify(selectedAccessGroup)
                        ),
                      },
                    }} disableRipple disabled={buttonBlock}>
                      <EditIcon />
                      &#8288;Edit
                    </MenuItem>
                  <MenuItem
