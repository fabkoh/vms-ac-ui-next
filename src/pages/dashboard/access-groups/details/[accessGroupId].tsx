...
const AccessGroupDetails = () => {
  ...
  const getAccessGroupInSchedule = async () => {
    const res =
      await accessGroupScheduleApi.getAccessGroupStatusForSingleAccessGroup(
        accessGroupId
      );
    if (res.status != 200) {
  ...
  const deleteSchedules = async (ids) => {
    const resArr = await Promise.all(
      ids.map(accessGroupScheduleApi.deleteAccessGroupSchedule)
    );
  ...
  const handleStatusUpdate = async (accessGroupId, updatedStatus) => {
    ...
    Promise.resolve(
      ...
      if (res.status == 200) {
        ...
        toast.error(
          "Failed to " +
            (updatedStatus ? "activate" : "deactivated") +
            " access group"
        );
      }
    });
  ...
  return (
    <>
      <Head>
        <title>Etlas: Access Group Details</title>
      </Head>
      <ConfirmStatusUpdate
        ...
        <Container maxWidth="md">
          <div>
            <Box sx={{ mb: 4 }}>
                ...
                <Link
                  color="textPrimary"
                  component={NextLink} href={accessGroupListLink}
                  sx={{
                    alignItems: "center",
                    display: "flex",
                  }}
            ...
            <Grid container justifyContent="space-between" spacing={3}>
              ...
              <Grid item sx={{ m: -1 }}>
                ...
                >
                    ...
                    <MenuItem component={NextLink} href={accessGroupCreateLink} disableRipple>
                      <AddIcon />
                      &#8288;Create
                    </MenuItem>
                  
                    <MenuItem component={NextLink} href={getAccessGroupEditLink(accessGroup)} disableRipple>
                      <EditIcon />
                      &#8288;Edit
                    </MenuItem>
                  <MenuItem
