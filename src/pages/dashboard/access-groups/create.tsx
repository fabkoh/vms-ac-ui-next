...
const CreateAccessGroups = () => {
  ...
  const addCard = () => {
    ...
    setAccessGroupInfoArr([
      ...accessGroupInfoArr,
      getEmptyAccessGroupInfo(newId),
    ]);
    setAccessGroupValidationsArr([
      ...accessGroupValidationsArr,
      getEmptyAccessGroupValidations(newId),
    ]);
  };
  ...
  const submitForm = (e) => {
    ...
    setSubmitted(true);
    Promise.all(
      accessGroupInfoArr.map((accessGroup) =>
        accessGroupApi.createAccessGroup(accessGroup)
      )
    ).then((resArr) => {
      ...
      Promise.all(successResIndex.map((i) => resArr[i].json())).then(
        (successResJson) => {
          ...
          successResIndex.forEach((index, i) => {
            const entranceIds = originalAccessGroupInfoArr[index].entrances.map(
              (e) => e.entranceId
            );
            if (entranceIds.length > 0) {
              // there are entrances to add
          ...
          Promise.all(
            accessGroupToEntrance.map((groupToEntrance) =>
              accessGroupEntranceApi.assignEntrancesToAccessGroup(
                groupToEntrance[1],
                groupToEntrance[0]
              )
            )
            ...
            (resArr) => {
              resArr.forEach((res, i) => {
                if (res.status != 204) {
                  ...
                  toast.error(
                    `Failed to assign entrances to ${accessGroupName}`
                  );
                }
              });
            };
      ...
      if (failedResIndex.length) {
        ...
        Promise.all(failedResIndex.map((i) => resArr[i].json())).then(
          (failedResArr) => {
            ...
            setAccessGroupInfoArr(
              failedResIndex.map((i) => accessGroupInfoArr[i])
            ); // set failed access groups to stay
            setAccessGroupValidationsArr(
              failedResIndex.map((i) => accessGroupValidationsArr[i])
            ); // set failed access group validations to stay
          }
        );
      }
    });
  ...
  return (
    <>
      <Head>
        <title>Etlas: Create Access Groups</title>
      </Head>
      <Box
        component="main"
        sx={{
      ...
      >
        ...
        <Container maxWidth="xl">
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Create Access Groups</Typography>
          </Box>
