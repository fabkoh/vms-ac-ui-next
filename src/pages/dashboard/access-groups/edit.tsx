...
const EditAccessGroups = () => {
  ...
  const getAccessGroups = async (ids) => {
    ...
    const resArr = await Promise.all(
      ids.map((id) => accessGroupApi.getAccessGroup(id))
    );
    ...
    if (serverDownRes.length > 0) {
      toast.error(
        "Error editing access groups due to server is down. Please try again"
      );
  ...
  const getGroupEntrances = async (groups) => {
    ...
    const resArr = await Promise.all(
      accessGroups.map((group) =>
        accessGroupEntranceApi.getEntranceWhereAccessGroupId(
          group.accessGroupId
        )
      )
    );
    ...
    if (serverDownArr.length > 0) {
      toast.error(
        "Error getting entrances due to server is down. Please try again"
      );
      router.replace("/dashboard/access-groups");
      return;
    }
    ...
    resArr.forEach((res, i) => {
      if (res.status == 200) {
        ...
        toast.error(
          `Entrances for ${accessGroups[i].accessGroupName} not loaded. Please clear to prevent changes to entrance`
        );
      }
    });
    const bodyArr = await Promise.all(
      successfulResIndex.map((i) => resArr[i].json())
    );
    bodyArr.forEach((body, i) => {
      accessGroups[successfulResIndex[i]].entrances = body.map(
        (e) => e.entrance
      );
    });
  ...
  useEffect(() => {
    accessGroupApi.getAccessGroups().then(async (res) => {
      ...
      if (res.status == 200) {
        const body = await res.json();
        body.forEach(
          (group) => (newAccessGroupNames[group.accessGroupName] = true)
        );
  ...
  const checkDuplicateName = (groupArr, validationArr) => {
    ...
    for (let i = 0; i < groupArr.length; i++) {
      validationArr[i].accessGroupNameDuplicated =
        groupArr[i].accessGroupName in duplicatedNames;
    }
  };
  ...
  // remove card logic
  const removeCard = (id) => {
    const newAccessGroupInfoArr = accessGroupInfoArr.filter(
      (info) => info.accessGroupId != id
    );
    const newValidations = accessGroupValidationsArr.filter(
      (validation) => validation.accessGroupId != id
    );
    ...
    setDuplicatedPerson(
      checkDuplicatePerson(newAccessGroupInfoArr, newValidations)
    );
  ...
  const changeTextField = (e, id) => {
    ...
    updatedInfo.find((info) => info.accessGroupId == id)[e.target.name] =
      e.target.value;
  ...
  const changeNameCheck = async (e, id) => {
    ...
    const newCurrAccessGroup = newAccessGroupInfoArr.find(
      (group) => group.accessGroupId == id
    );
  ...
  const changePersonCheck = (newValue, id) => {
    ...
    const newAccessGroupInfoArr = [...accessGroupInfoArr];
    const newAccessGroupInfo = newAccessGroupInfoArr.find(
      (group) => group.accessGroupId == id
    );
    ...
    validation.accessGroupPersonHasAccessGroup = newValue.some(
      (person) =>
        person.accessGroup && !originalPersonIds.includes(person.personId)
    );

    // check person duplicated
    setDuplicatedPerson(
      checkDuplicatePerson(newAccessGroupInfoArr, validations)
    );
  ...
  const submitForm = async (e) => {
    ...
    const resArr = await Promise.all(
      accessGroupInfoArr.map((group) => accessGroupApi.updateAccessGroup(group))
    );
    ...
    setAccessGroupInfoArr(
      accessGroupInfoArr.filter((e, i) => !successStatus[i])
    );
    setAccessGroupValidationsArr(
      accessGroupValidationsArr.filter((e, i) => !successStatus[i])
    );
    setSubmitted(false);
  };
  ...
  return (
    <>
      <Head>
        <title>Etlas: Edit Access Groups</title>
      </Head>
      <Box
        component="main"
        sx={{
      ...
      >
        <Container maxWidth="xl">
          ...
          <Box marginBottom={3}>
            <Typography variant="h3">Edit Access Groups</Typography>
          </Box>
            ...
            <Stack spacing={3}>
              ...
              <Grid container>
                <Grid item marginRight={3}>
                  <Button
                    ...
                    disabled={
                      ...
                      accessGroupValidationsArr.some(
                        // check if validations fail
                        (validation) =>
                          validation.accessGroupNameBlank ||
                          validation.accessGroupNameExists ||
                ...
                <Grid item>
                    ...
                    <Button component={NextLink} href="/dashboard/access-groups/"
                      size="large"
                      variant="outlined"
                      color="error"
                      onClick={() => router.back()}
